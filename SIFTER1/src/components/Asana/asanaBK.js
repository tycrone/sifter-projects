export let getAsanaData = (squadTagId, milestoneCode) =>{
    ...
    return new Promise((returnDat) => {

        //1. GET ALL TASKS FROM TEAM TAG (1 API CALL TOTAL)
        const acquireTaskIds = new Promise((resolve) => {
            let allTaskIds = [];
        ...
        acquireTaskIds.then(acquireProjects)
    
    }).then(()=>{return ganttData})
}
